...
interface AdmTeacherTableProps {
  teachers: Teacher[];
  loading: boolean;
  currentPage: number;
  totalItems: number;
...
export default function AdmTeacherTable({
  teachers,
  loading,
  currentPage,
  totalItems,
  ...
  if (teachers.length === 0) {
    return (
      <div className="dark-gradient rounded-2xl p-8 border border-input text-center">
        <p className="text-light-100">Hozircha o'qituvchilar mavjud emas</p>
      </div>
    );
  }
              ...
              >
                ...
                <td className="px-4 md:px-6 py-3 md:py-4 whitespace-nowrap">
                  <select
                    ...
                    onChange={(e) =>
                      onStatusChange(teacher.id, e.target.value as "active" | "inactive")
                    }
                ...
                <td className="px-4 md:px-6 py-3 md:py-4 text-right whitespace-nowrap">
                  <div className="flex gap-2 justify-end">
                    ...
                    <Button onClick={() => onDelete(teacher.id)} variant="destructive" size="sm">
